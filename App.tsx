...
import {
  ...
  Sparkles, Clock, Dog, Plus, AlertCircle, LogOut
} from 'lucide-react';
...
const App: React.FC<{ onLogout?: () => void }> = ({ onLogout }) => {
                ...
                return (
                  <Reorder.Item key={moduleId} value={moduleId}>
                    <div className="relative bg-white/50 rounded-2xl border border-wood-100">
                      ...
                      <div className="pl-8">
                        <WidgetSlot title={module.title} icon={module.icon} isLoaded={isLoaded} isPlaceholder />
                      </div>
                    </div>
                  </Reorder.Item>
                );
  ...
  const renderMeTab = () => (
    <div className="animate-[fadeIn_0.3s_ease-out] space-y-8 pb-24 pt-4">
      ...
      <button
        onClick={onLogout}
        className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl text-red-400 text-sm font-medium hover:bg-red-50 active:scale-[0.98] transition-all border border-red-100"
      >
        <LogOut size={16} />
        退出登录
      </button>
